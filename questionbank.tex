	...
	\begin{question}{q03.1}
		...
		\begin{center}
			\begin{tikzpicture}
				...
				\draw[very thick,-Stealth] (2,1) coordinate(B) -- +(40:3) coordinate(A);
				\node[above right] at (A) {\(\vec{u}\)};
				\draw[help lines,dashed] (B) -- ++(0,2) coordinate(C);
				\pic["\ang{50}",draw,angle eccentricity=2] {angle};
			\end{tikzpicture}
		\end{center}
		\begin{multicols}{3}\AMCBoxedAnswers{}
			\begin{choices}
				\wrongchoice{\(\begin{pmatrix}\qty{0.77}{\meter} \\ \qty{0.64}{\meter}\end{pmatrix}\)}
				\wrongchoice{\(\begin{pmatrix}\qty{1.29}{\meter} \\ \qty{1.53}{\meter}\end{pmatrix}\)}
				\correctchoice{\(\begin{pmatrix}\qty{1.53}{\meter} \\ \qty{1.29}{\meter}\end{pmatrix}\)}
				\wrongchoice{\(\begin{pmatrix}\qty{2}{\meter} \\ \qty{1.5}{\meter}\end{pmatrix}\)}
				\lastchoices{}\columnbreak{}
				\wrongchoice{aucune}
				\wrongchoice{toutes}
				\wrongchoice{manque}
